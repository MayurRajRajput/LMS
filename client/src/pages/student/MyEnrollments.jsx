...
const MyEnrollments = () => {
  const {enrolledCourses,fetchUserEnrolledCourses,calculateCourseDuration,navigate,userData,backendUrl,getToken,calculateNoOfLectures} = useContext(AppContext)
            ...
            {enrolledCourses.map((course,index)=>(
              <tr className='border-b border-gray-500/20' key={index}>
                ...
                <td className='px-4 py-3 max-sm:text-right'>
                  <button onClick={()=>navigate('/player/'+course._id)} className='px-3 rounded sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white'>
